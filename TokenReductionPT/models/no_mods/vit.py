...
from .attention_rollout import calc_attention_map
from .augment_images import batch_augment, generate_combinations
...
class ViT(nn.Module):
    ...
    def __init__(
        ...
        self.da_aug = getattr(args, 'da_aug', None)
        self.thresh_max = getattr(args, 'aug_thresh_max', False)
        self.aug_thresh_crop = getattr(args, 'aug_thresh_crop', None)
        self.aug_thresh_mask = getattr(args, 'aug_thresh_mask', None)
        self.aug_topk_crop = getattr(args, 'aug_topk_crop', None)
        self.aug_topk_mask = getattr(args, 'aug_topk_mask', None)
        da_aug_layers = getattr(args, 'da_aug_layers', [])
        if da_aug_layers:
            self.da_aug_layers = da_aug_layers
            print('Set layer for data-aware aug: ', self.da_aug_layers)
        elif self.da_aug:
            #self.combinations = generate_combinations(1, self.reduction_loc[0])
            self.combinations = generate_combinations(0, self.reduction_loc[0])
            print('Random layers from the following set: ', self.combinations)
        da_pool = getattr(args, 'da_pool', 0)
        if da_pool:
            self.aug_pool = nn.AvgPool2d(da_pool, da_pool)
        ...
        ifa_head = getattr(args, 'ifa_head', False)
        if ifa_head:
            self.kernels_per_layer = getattr(args, 'ifa_dws_conv_groups', 4)

            if self.kernels_per_layer:
                conv = nn.Sequential(
                    nn.Conv1d(self.embed_dim, self.embed_dim * self.kernels_per_layer,
                             len(self.reduction_loc) + 1, len(self.reduction_loc) + 1, groups=self.embed_dim),
                    nn.Conv1d(self.embed_dim * self.kernels_per_layer, num_classes, 1),
                )
            else:
                conv = nn.Conv1d(self.embed_dim, num_classes, len(self.reduction_loc) + 1, len(self.reduction_loc) + 1)
        ...
        self.reweight_intermediate_cls = getattr(args, 'reweight_intermediate_cls', False)
    ...
    def reset_classifier(self, num_classes: int, global_pool = None) -> None:
        if hasattr(self, 'ifa_head'):
            ...
            if not self.reweight_intermediate_cls:
