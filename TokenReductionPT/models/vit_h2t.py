...
__all__ = ['H2TViT']  # model_registry will add each entrypoint fn to this
...
class Block(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        h1 = self.norm1(x)
        h2 = self.drop_path1(self.ls1(self.attn(h1)))
        x = x + h2

        h4, h3 = self.mlp(self.norm2(x))
        h4 = self.drop_path2(self.ls2(h4))
        x = x + h4
        inter = [reduce(ft, 'b s d -> b d', 'mean') for ft in (h1, h2, h3, h4)]
        inter = F.normalize(torch.cat(inter, dim=1), dim=-1)
        return x, inter
...
class H2TViT(nn.Module):
    ...
    def __init__(
        ...
        # default inter locs
        inter_loc = getattr(args, 'reduction_loc', None)
        if inter_loc:
            self.inter_loc = inter_loc
        else:
            self.inter_loc = [i for i in range(depth)]
        channels_per_layer = (embed_dim * 3) + int((embed_dim * mlp_ratio))
        self.total_channels = (len(self.inter_loc) * channels_per_layer) + (2 * embed_dim)
        print(f'H2T with {self.inter_loc} layers and {self.total_channels} total channels')
    ...
    def reset_classifier(self, num_classes: int, global_pool = None) -> None:
        ...
        if global_pool is not None:
            ...
            self.global_pool = global_pool
        self.head = nn.Linear(self.total_channels, num_classes) if num_classes > 0 else nn.Identity()
        return 0
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        inter_feat_list = []
        inter_feat_list.append(F.normalize(reduce(x, 'b s d -> b d', 'mean'), dim=-1))

        for i, blck in enumerate(self.blocks):
            x, inter = blck(x)

            if i in self.inter_loc:
                inter_feat_list.append(inter)

        x = self.norm(x)
        inter_feat_list.append(F.normalize(reduce(x, 'b s d -> b d', 'mean'), dim=-1))

        x = torch.cat(inter_feat_list, dim=1)
...
@torch.no_grad()
def _load_weights(model: H2TViT, checkpoint_path: str, prefix: str = '') -> None:
...
default_cfgs = {
    ...
    'h2tvit_base_patch16_224.orig_in21k': _cfg(
    ...
    'h2tvit_small_patch14_reg4_dinov2.lvd142m': _cfg(
        ...
        mean=IMAGENET_DEFAULT_MEAN, std=IMAGENET_DEFAULT_STD, num_classes=0,
        input_size=(3, 518, 518), crop_pct=1.0),
    'h2tvit_base_patch14_reg4_dinov2.lvd142m': _cfg(
...
@register_model
def h2tvit_tiny_patch16_224(pretrained: bool = False, **kwargs):
    """ ViT-Tiny (Vit-Ti/16)
    """
    model_args = dict(patch_size=16, embed_dim=192, depth=12, num_heads=3)
    model = _create_vision_transformer('h2tvit_tiny_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def h2tvit_small_patch16_224(pretrained: bool = False, **kwargs):
    """ ViT-Small (ViT-S/16)
    """
    model_args = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6)
    model = _create_vision_transformer('h2tvit_small_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def h2tvit_small_patch8_224(pretrained: bool = False, **kwargs):
    """ ViT-Small (ViT-S/8)
    """
    model_args = dict(patch_size=8, embed_dim=384, depth=12, num_heads=6)
    model = _create_vision_transformer('h2tvit_small_patch8_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def h2tvit_base_patch16_224(pretrained: bool = False, **kwargs):
    """ ViT-Base (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-1k weights fine-tuned from in21k @ 224x224, source https://github.com/google-research/vision_transformer.
    """
    model_args = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12)
    model = _create_vision_transformer('h2tvit_base_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def h2tvit_base_patch8_224(pretrained: bool = False, **kwargs):
    """ ViT-Base (ViT-B/8) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-1k weights fine-tuned from in21k @ 224x224, source https://github.com/google-research/vision_transformer.
    """
    model_args = dict(patch_size=8, embed_dim=768, depth=12, num_heads=12)
    model = _create_vision_transformer('h2tvit_base_patch8_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def h2tvit_large_patch16_224(pretrained: bool = False, **kwargs):
    """ ViT-Large model (ViT-L/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-1k weights fine-tuned from in21k @ 224x224, source https://github.com/google-research/vision_transformer.
    """
    model_args = dict(patch_size=16, embed_dim=1024, depth=24, num_heads=16)
    model = _create_vision_transformer('h2tvit_large_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def h2tvit_large_patch14_224(pretrained: bool = False, **kwargs):
    """ ViT-Large model (ViT-L/14)
    """
    model_args = dict(patch_size=14, embed_dim=1024, depth=24, num_heads=16)
    model = _create_vision_transformer('h2tvit_large_patch14_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def h2tvit_huge_patch14_224(pretrained: bool = False, **kwargs):
    """ ViT-Huge model (ViT-H/14) from original paper (https://arxiv.org/abs/2010.11929).
    """
    model_args = dict(patch_size=14, embed_dim=1280, depth=32, num_heads=16)
    model = _create_vision_transformer('h2tvit_huge_patch14_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def h2tvit_base_patch16_224_miil(pretrained: bool = False, **kwargs):
    """ ViT-Base (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    Weights taken from: https://github.com/Alibaba-MIIL/ImageNet21K
    """
    model_args = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, qkv_bias=False)
    model = _create_vision_transformer(
        'h2tvit_base_patch16_224_miil', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def h2tvit_medium_patch16_gap_240(pretrained: bool = False, **kwargs):
    ...
    model = _create_vision_transformer(
        'h2tvit_medium_patch16_gap_240', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def h2tvit_medium_patch16_gap_256(pretrained: bool = False, **kwargs):
    ...
    model_args = dict(
        ...
        global_pool='avg', qkv_bias=False, init_values=1e-6, fc_norm=False)
    model = _create_vision_transformer(
        'h2tvit_medium_patch16_gap_256', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def h2tvit_base_patch16_gap_224(pretrained: bool = False, **kwargs):
    ...
    model = _create_vision_transformer(
        'h2tvit_base_patch16_gap_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def h2tvit_huge_patch14_gap_224(pretrained: bool = False, **kwargs):
    ...
    model = _create_vision_transformer(
        'h2tvit_huge_patch14_gap_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def h2tvit_giant_patch16_gap_224(pretrained: bool = False, **kwargs):
    ...
    model = _create_vision_transformer(
        'h2tvit_giant_patch16_gap_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def h2tvit_base_patch16_clip_224(pretrained: bool = False, **kwargs):
    """ ViT-B/16 CLIP image tower
    """
    model_args = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, pre_norm=True, norm_layer=nn.LayerNorm)
    model = _create_vision_transformer(
        'h2tvit_base_patch16_clip_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model
...
@register_model
def h2tvit_small_patch14_dinov2(pretrained: bool = False, **kwargs):
    """ ViT-S/14 for DINOv2
    """
    model_args = dict(patch_size=14, embed_dim=384, depth=12, num_heads=6, init_values=1e-5, img_size=518)
    model = _create_vision_transformer(
        'h2tvit_small_patch14_dinov2', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def h2tvit_base_patch14_dinov2(pretrained: bool = False, **kwargs):
    """ ViT-B/14 for DINOv2
    """
    model_args = dict(patch_size=14, embed_dim=768, depth=12, num_heads=12, init_values=1e-5, img_size=518)
    model = _create_vision_transformer(
        'h2tvit_base_patch14_dinov2', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def h2tvit_large_patch14_dinov2(pretrained: bool = False, **kwargs):
    """ ViT-L/14 for DINOv2
    """
    model_args = dict(patch_size=14, embed_dim=1024, depth=24, num_heads=16, init_values=1e-5, img_size=518)
    model = _create_vision_transformer(
        'h2tvit_large_patch14_dinov2', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def h2tvit_giant_patch14_dinov2(pretrained: bool = False, **kwargs):
    """ ViT-G/14 for DINOv2
    """
    ...
    model = _create_vision_transformer(
        'h2tvit_giant_patch14_dinov2', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def h2tvit_small_patch14_reg4_dinov2(pretrained: bool = False, **kwargs):
    ...
    model = _create_vision_transformer(
        'h2tvit_small_patch14_reg4_dinov2', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def h2tvit_base_patch14_reg4_dinov2(pretrained: bool = False, **kwargs):
    ...
    model = _create_vision_transformer(
        'h2tvit_base_patch14_reg4_dinov2', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def h2tvit_large_patch14_reg4_dinov2(pretrained: bool = False, **kwargs):
    ...
    model = _create_vision_transformer(
        'h2tvit_large_patch14_reg4_dinov2', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def h2tvit_giant_patch14_reg4_dinov2(pretrained: bool = False, **kwargs):
    """ ViT-G/14 for DINOv2
    """
    # The hidden_features of SwiGLU is calculated by:
    # hidden_features = (int(hidden_features * 2 / 3) + 7) // 8 * 8
    ...
    model = _create_vision_transformer(
        'h2tvit_giant_patch14_reg4_dinov2', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def h2tvit_base_patch16_siglip_224(pretrained: bool = False, **kwargs):
    model_args = dict(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, class_token=False, global_pool='map',
    )
    model = _create_vision_transformer(
        'h2tvit_base_patch16_siglip_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def h2tvit_base_patch16_siglip_256(pretrained: bool = False, **kwargs):
    model_args = dict(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, class_token=False, global_pool='map',
    )
    model = _create_vision_transformer(
        'h2tvit_base_patch16_siglip_256', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def h2tvit_base_patch16_siglip_512(pretrained: bool = False, **kwargs):
    model_args = dict(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, class_token=False, global_pool='map',
    )
    model = _create_vision_transformer(
        'h2tvit_base_patch16_siglip_512', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def h2tvit_large_patch16_siglip_256(pretrained: bool = False, **kwargs):
    model_args = dict(
        patch_size=16, embed_dim=1024, depth=24, num_heads=16, class_token=False, global_pool='map',
    )
    model = _create_vision_transformer(
        'h2tvit_large_patch16_siglip_256', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def h2tvit_medium_patch16_reg4_256(pretrained: bool = False, **kwargs):
    model_args = dict(
        patch_size=16, embed_dim=512, depth=12, num_heads=8, class_token=True,
        no_embed_class=True, reg_tokens=4,
    )
    model = _create_vision_transformer(
        'h2tvit_medium_patch16_reg4_256', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def h2tvit_medium_patch16_reg4_gap_256(pretrained: bool = False, **kwargs):
    model_args = dict(
        patch_size=16, embed_dim=512, depth=12, num_heads=8,
        class_token=False, no_embed_class=True, reg_tokens=4, global_pool='avg',
    )
    model = _create_vision_transformer(
        'h2tvit_medium_patch16_reg4_gap_256', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def h2tvit_base_patch16_reg8_gap_256(pretrained: bool = False, **kwargs):
    model_args = dict(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, class_token=False,
        no_embed_class=True, global_pool='avg', reg_tokens=8,
    )
    model = _create_vision_transformer(
        'h2tvit_base_patch16_reg8_gap_256', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def h2tdeit_tiny_patch16_224(pretrained=False, **kwargs):
    """ DeiT-tiny model @ 224x224 from paper (https://arxiv.org/abs/2012.12877).
    ImageNet-1k weights from https://github.com/facebookresearch/deit.
    """
    model_args = dict(patch_size=16, embed_dim=192, depth=12, num_heads=3)
    model = _create_vision_transformer('h2tdeit_tiny_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def h2tdeit_small_patch16_224(pretrained=False, **kwargs):
    """ DeiT-small model @ 224x224 from paper (https://arxiv.org/abs/2012.12877).
    ImageNet-1k weights from https://github.com/facebookresearch/deit.
    """
    model_args = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6)
    model = _create_vision_transformer('h2tdeit_small_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def h2tdeit_base_patch16_224(pretrained=False, **kwargs):
    """ DeiT base model @ 224x224 from paper (https://arxiv.org/abs/2012.12877).
    ImageNet-1k weights from https://github.com/facebookresearch/deit.
    """
    model_args = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12)
    model = _create_vision_transformer('h2tdeit_base_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def h2tdeit3_small_patch16_224(pretrained=False, **kwargs):
    ...
    model = _create_vision_transformer('h2tdeit3_small_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def h2tdeit3_medium_patch16_224(pretrained=False, **kwargs):
    ...
    model = _create_vision_transformer('h2tdeit3_medium_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def h2tdeit3_base_patch16_224(pretrained=False, **kwargs):
    ...
    model = _create_vision_transformer('h2tdeit3_base_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def h2tdeit3_large_patch16_224(pretrained=False, **kwargs):
    ...
    model = _create_vision_transformer('h2tdeit3_large_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def h2tdeit3_huge_patch14_224(pretrained=False, **kwargs):
    ...
    model = _create_vision_transformer('h2tdeit3_huge_patch14_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model
